...
function handshaking($newClient, $line){
    ...
    $upgrade  = "HTTP/1.1 101 Web Socket Protocol Handshake\r\n" .
        ...
        "WebSocket-Location: ws://0.0.0.0:12346/server.php\r\n".
...
function game_over(&$cur_client, &$room, &$room_list, &$r_key, $giveup)
{
    ...
    //存战绩
    $sql = "insert into record (user1, user2, type) values ('$winner', '$loser', '五子棋')";
    $con->query($sql);
    $sql = "update wzqRanking set score=score+20, wins=wins+1 where username='$winner'";
    $con->query($sql);
    $sql = "update wzqRanking set score=score-20, loses=loses+1 where username='$loser'";
    $con->query($sql);
...
socket_bind($server, '10.0.2.15', 12346);
